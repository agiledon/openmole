# OpenMole CLI `openmole init` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `openmole init` CLI that bootstraps `openmole/` workspace and configures Cursor + OpenCode (Phase A) via interactive IDE multiselect.

**Architecture:** Single-package `cli/` + `bin/openmole.js`; `@clack/prompts` for TUI; per-IDE adapters with hybrid scope (Cursor user-level, OpenCode project-level); state in `openmole/config.yaml` (`installed_ides`, `init_version`, `init_at`).

**Tech Stack:** Node.js ESM, `@clack/prompts`, zero-dep plugin (`openmole.js`/skills unchanged)

**Design:** [2026-06-05-openmole-cli-init-design.md](../2026-06-05-openmole-cli-init-design.md)
...
| `cli/lib/package-root.js` | Resolve OpenMole package root |
...
**Files:**
- Create: `bin/openmole.js`, `cli/index.js`
- Modify: `package.json`
...
- [ ] **Step 4:** Run `chmod +x bin/openmole.js && npm link && openmole --help`

Expected: prints usage

- [ ] **Step 5:** Commit: `feat(cli): add openmole bin entry and command router`
...
- [ ] **Step 1:** Implement `resolvePackageRoot()` using `OPENMOLE_HOME` → `import.meta.url` dirname walk → optional global fallback

- [ ] **Step 2:** Test asserts path contains `skills/` and `.cursor-plugin/plugin.json`

- [ ] **Step 3:** Commit: `feat(cli): add OpenMole package root resolver`
...
- Modify: `templates/openmole-config.yaml.example` (done in design)
- Test: `tests/cli/test-init-workspace.sh`

- [ ] **Step 1:** Write test: temp dir, run `openmole init --ides none`, assert dirs + yaml fields

- [ ] **Step 2:** Implement bootstrap (minimal YAML writer without extra deps — template string or line-based)

- [ ] **Step 3:** Run test → PASS

- [ ] **Step 4:** Commit: `feat(cli): bootstrap openmole workspace and config metadata`
...
- [ ] **Step 1:** Read/merge `opencode.json` `plugin` array with abs path to `openmole.js`
...
- [ ] **Step 5:** Commit: `feat(cli): wire openmole init orchestration with extend and dry-run`
...
- [ ] **Step 1:** README primary install: `npm link` (dev) / `npm install -g openmole` (target)

- [ ] **Step 2:** `install-cursor-plugin.sh` → `openmole init --ides cursor`

- [ ] **Step 3:** Commit: `docs: document openmole init as primary install path`
