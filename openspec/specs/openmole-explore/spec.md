...
Scan source and produce badsmells.md within a OpenMole change directory.

## Requirements

### Requirement: openmole-explore scans source and produces badsmells.md

The `openmole-explore` skill and `mole:explore` command SHALL scan a user-specified directory, identify bad smells aligned with constitution §3 and specification §4, and write output to `openmole/changes/<change-name>/badsmells.md`.
...
Before adding entries, explore SHALL load fingerprints from `openmole/changes/` and `openmole/changes/archive/` and SHALL NOT duplicate smells already tracked.
