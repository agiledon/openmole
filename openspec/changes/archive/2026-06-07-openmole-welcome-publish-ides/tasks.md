...
- [x] 1.1 新增 `cli/prompts/welcome.js`：ASCII R logo、青色闪烁动画、OpenMole 文案与 Quick start
- [x] 1.2 TTY 检测；非 TTY 跳过 welcome
- [x] 1.3 `cli/index.js`：无参数 `openmole` → welcome → `runInit`
...
- [x] 3.1 `package.json`：`name` → `openmole`，bump 版本（如 0.5.0）
- [x] 3.2 更新 README 中英文：registry 安装 `npm install -g openmole`
- [x] 3.3 更新 `test-package-publish.sh` 断言包名
- [x] 3.4 `npm publish --access public`（需 npm login）
- [x] 3.5 验证 `npm install -g openmole` 后 `openmole --version`

## 4. 文档与验收

- [x] 4.1 更新 `templates/openmole-config.yaml.example` installed_ides 注释（7 IDE）
- [x] 4.2 `bash scripts/validate-cli.sh` 全绿
- [x] 4.3 手动：TTY 运行 `openmole` → 见闪烁 R → Enter → 7 IDE 清单
