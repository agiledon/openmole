...
The `openmole-verify` skill and `mole:verify` command SHALL compare `{docs_root}/badsmells.md` entries and versions against `{docs_root}/tasks.md` to find coverage gaps, obsolete tasks, dependency conflicts, and DoD mismatches. Output SHALL be recorded in `{docs_root}/analysis.md`.
