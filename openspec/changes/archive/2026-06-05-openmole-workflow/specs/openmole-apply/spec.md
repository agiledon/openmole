...
The `openmole-apply` skill and `mole:apply` command SHALL read `{docs_root}/tasks.md`, select the next task or step marked **未执行**, and execute it following constitution §4 and §5 without skipping test-green-refactor-verify loop.
...
Each `mole:apply` invocation SHALL complete at most one task (or one logical step group within a task) and SHALL pause for user confirmation before suggesting the next apply, aligning with constitution §4 step 5 and tasks.md confirmation gate.
