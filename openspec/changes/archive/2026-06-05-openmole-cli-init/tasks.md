...
- [x] 1.1 新增 `bin/openmole.js`、`cli/index.js` 路由（init / --help / --version）
- [x] 1.2 `package.json`：`bin.openmole`；`dependencies: { "@clack/prompts": "..." }`（plugin 不引用）
- [x] 1.3 `cli/commands/init.js`：解析 `--ides` / `--all` / `--none` / `--force` / `--global` / `--dry-run`

## 2. config.yaml 与 workspace

- [x] 2.1 更新 `templates/openmole-config.yaml.example`：`installed_ides`、`init_version`、`init_at`
- [x] 2.2 `cli/lib/config-yaml.js`：读/写/合并 YAML（零依赖）
- [x] 2.3 `cli/workspace/bootstrap.js`：创建 `openmole/changes/`、`openmole/changes/archive/`
...
- [x] 4.1 `cli/lib/package-root.js`：`OPENMOLE_HOME` / `import.meta.url`
- [x] 4.2 `cli/adapters/cursor.js`：用户级 symlink → `~/.cursor/plugins/local/openmole`
...
- [x] 7.1 README：`npm link` 开发 + 目标 `npm install -g openmole && openmole init`
- [x] 7.2 `install-cursor-plugin.sh` 委托 `openmole init --ides cursor`
- [x] 7.3 更新 `.cursor/INSTALL.md`、`.opencode/INSTALL.md`、`.claude/INSTALL.md`、`.codex/INSTALL.md`、`.gemini/INSTALL.md`

## 8. 验收

- [x] 8.1 `openmole init --ides opencode` → workspace + opencode.json
- [x] 8.2 `openmole init --ides cursor` → workspace + symlink
- [x] 8.3 交互式双选 IDE（手动，可选验收 — 非阻塞）
- [x] 8.4 extend：二次 init 追加 IDE，`config.yaml` 保留

## Phase 2

- [x] 9.1 `openmole update`：按 `installed_ides` 重装 IDE 配置并刷新 `init_version`
- [x] 9.2 `templates/openmole-gitignore.snippet` + init/update 合并到项目 `.gitignore`
