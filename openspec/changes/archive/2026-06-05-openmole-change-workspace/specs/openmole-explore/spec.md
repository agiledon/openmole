...
The `openmole-explore` skill and `mole:explore` command SHALL scan a user-specified directory or project root, identify code bad smells aligned with constitution §3 and specification §4, and write output to `openmole/changes/<change-name>/badsmells.md`. Each `mole:explore` invocation that starts a new OpenMole cycle SHALL create or designate a **new change** unless the user explicitly continues the current change.
...
- **WHEN** the user runs `mole:explore src/auth` without a change name
...
Before adding new bad smell entries, explore SHALL load BS-ID and location fingerprints from all prior changes under `openmole/changes/` (including `archive/`) and SHALL NOT create duplicate entries for the same file location and smell class already tracked in another change.
