...
The `openmole-apply` skill and `mole:apply` command SHALL read `openmole/changes/<current-change>/tasks.md`, select the next task marked **未执行**, and execute it following constitution §4 and §5.

#### Scenario: Apply within current change

- **WHEN** the user runs `mole:apply` and `openmole/config.yaml` specifies `current_change: refactor-utils`
